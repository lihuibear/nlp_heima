...
class NameDataset(Dataset):
    def __init__(self, my_list_x, my_list_y):
        super().__init__()
        # 获取x
        self.my_list_x = my_list_x  # 存储人名
        # 获取y
        self.my_list_y = my_list_y  # 存储国家类型
        # 获取样本的数量
    ...
    def __getitem__(self, item):
        ...
        # print(f'y--->{y}')
        # 3.将人名变成one-hot编码的张量形式 张量为[人名长度(单词数),所有的字符总量（26*2+5）]
        # 3.1 初始化全零的张量 #(3, 57)
        ...
        for idx, letter in enumerate(x):
            ...
            # one-hot编码 原理是为每个字符创建一个「长度等于字符总类别数（n_letters=57）」的向量
            # 向量中仅有对应字符的索引位置值为 1，其余所有位置的值均为 0，
            tensor_x[idx][all_letters.find(letter)] = 1
            # print(f'tensor_x--》{tensor_x}')
        # print(f'tensor_x修改之后的：-->{tensor_x}')

        # 将表示国籍类别的字符串（如 "Chinese"），转换为 PyTorch 分类任务中模型可处理的「类别索引张量」，最终得到一个长整型（long）的标量张量，
...
def get_dataloader():
    ...
    # 封装dataset得到dataloader对象: 会对数据进行增加维度,shuffle= True 会随机打乱数据
        ...
        for idx, (x, y) in enumerate(tqdm(train_dataloader)):
            ...
            if total_iter_num % 2000 == 0:
                ...
                print('轮次:%d, 损失:%.6f, 时间:%d，准确率:%.3f' % (
                    epoch_idx + 1, temp_loss, time.time() - start_time, temp_acc))
        # 每轮都保存一个模型
